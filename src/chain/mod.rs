...
use crate::chain::bitcoind::common::{BoundedHeaderCache, ChainListener, FeeRateEstimationMode};
use crate::chain::bitcoind::BitcoindApiClient;
...
impl ChainSource {
	...
	pub(crate) fn new_bitcoind_rpc(
		...
		let api_client = Arc::new(BitcoindApiClient::new_rpc(
			rpc_host.clone(),
			rpc_port.clone(),
			rpc_user.clone(),
			rpc_password.clone(),
	...
	pub(crate) fn new_bitcoind_rest(
		...
		let api_client = Arc::new(BitcoindApiClient::new_rest(
	...
	pub(crate) async fn poll_and_update_listeners(
		...
		match self {
			...
			Self::Bitcoind {
				...
				let now = SystemTime::now();
				match api_client.get_mempool_transactions_and_timestamp_at_height(cur_height).await
				{
					Ok(unconfirmed_txs) => {
						log_trace!(
							logger,
